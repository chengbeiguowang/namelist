...
def scdex(input_namelist, file_base):
    ...
    for partition in race_array:
        ...
        if not os.path.isdir(dest_dir) or not os.path.exists(dest_excel):
            continue
        count += write_to_score_sheet(race_dict[partition], dest_excel, dest_dir)
    print(str(count))


def write_to_score_sheet(partition_dict, workbook_path, file_base) -> int:
    ...
    for key in partition_dict:
        partition_array = partition_dict[key]
        ...
        for i in range(len(partition_array)):
            ...
            link_file = ".\\" + key + BACK_SLASH + team_prefix + team.name + team_subfix + BACK_SLASH + get_program_file_name(
                team_base)
